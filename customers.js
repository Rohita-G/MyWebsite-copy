...
router.get("/", function (request, response, next) {
    ...
    queries.push(new Promise((resolve, reject) => {
        let query = "SELECT * FROM user_details ORDER BY userDetailsId DESC";
    ...
    // Address Data Query
    queries.push(new Promise((resolve, reject) => {
        let query = "SELECT * FROM user_address ORDER BY addressId DESC";
        connection.query(query, [loggedInUserId], function (error, data) {
            if (error) return reject(error);
    ...
    // Contacts Data Query
    queries.push(new Promise((resolve, reject) => {
        let query = "SELECT * FROM user_contacts ORDER BY contactId DESC";
        connection.query(query, [loggedInUserId], function (error, data) {
            if (error) return reject(error);
            resolve({ contactsData: data });
        });
...
router.get('/edit/details/:userDetailsId', function (request, response, next) {
    ...
    connection.query(query, function (error, data) {
        if (error) {
            ...
            response.render('customers', { title: 'Edit MySQL Table Data', action: 'edit', type: 'details', sampleData: data[0] });
        }
    });
...
router.get("/edit/address/:addressId", function (request, response, next) {
    ...
    var query = `SELECT * FROM user_address`;
...
router.get('/edit/contact/:contactId', function (request, response, next) {
    ...
    var query = `SELECT * FROM user_contacts`;
    ...
    connection.query(query, function (error, data) {
        if (error) {
            ...
            response.render('customers', { title: 'Edit MySQL Table Data', action: 'edit', type: 'contact', contactsData: data[0] });
        }
    });
